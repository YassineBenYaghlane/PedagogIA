    ...
    <form onSubmit={submit} className="flex flex-col border-t border-sage/15 bg-bone">
      <div className="flex items-end gap-2 p-3">
        <button
          ...
          className={`flex items-center justify-center w-10 h-10 rounded-xl border transition-colors duration-200 cursor-pointer disabled:cursor-not-allowed ${
            micActive
              ? "bg-rose/20 border-rose/40 text-rose animate-pulse"
              : "bg-chalk border-sage/20 text-sage-deep hover:bg-sage-leaf/30"
          } ${micBusy ? "opacity-50" : ""}`}
        >
          <Icon name={micBusy ? "progress_activity" : "mic"} size={18} className={micBusy ? "animate-spin" : ""} />
        </button>
        <textarea
          rows={1}
          value={value}
